            ...
            <table className="min-w-full divide-y divide-gray-300 border-t-2 border-gray-700">
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {data.map((player, idx) => (
                  <tr
                    key={player.name + idx}
                    className={`divide-x divide-gray-200 ${idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}
                  >
                    {TH_KEY.map((th, index) => {
                      return (
                        <td
                          key={th.key + index}
                          className={`${index === 0 ? `text-start` : 'text-center'} whitespace-nowrap border-none py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-0`}
                        >
                          {index === 0 ? `${idx + 1} ` : ' '}
                          {player[th.key]}
                        </td>
